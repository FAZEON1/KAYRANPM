...
# ════════════════════════════════════════════════════════════════════
    ...
    dc1.metric("G5F Depo", f"{secilen.get('bizim_stok', 0)} adet")
    firma_st = secilen.get("firma_stoklari", {})
    dc2.metric("Firmalarda", f"{secilen.get('toplam_firma_stok', 0)} adet")
    dc3.metric("Toplam Stok", f"{secilen.get('toplam_stok', secilen.get('bizim_stok', 0))} adet")
    dc4.metric("FOB Price", f"${secilen.get('fob_price', secilen.get('son_fob', 0)):,.2f}" if secilen.get('fob_price') or secilen.get('son_fob') else "—")
    dc5.metric("Cost Price", f"${secilen.get('cost_price', secilen.get('son_cost_price', 0)):,.2f}" if secilen.get('cost_price') or secilen.get('son_cost_price') else "—")
    dc6.metric("⭐ FINAL COST PRICE", f"${secilen.get('final_cost_price', 0):,.2f}" if secilen.get('final_cost_price') else "—",
